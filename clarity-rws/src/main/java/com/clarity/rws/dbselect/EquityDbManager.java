package com.clarity.rws.dbselect;
...
import com.clarity.rws.common.WeightRandom;
import com.clarity.rws.config.ConfigManager;
import com.clarity.rws.group.DataSourceWrapper;
import com.clarity.rws.util.NagiosUtils;
...
public class EquityDbManager extends AbstractDBSelector {
	...
	public EquityDbManager(Map<String, DataSourceWrapper> dataSourceWrapperMap, Map<String,/**权重值**/ Integer> weightMap) {
	...
	*/
	 ...
	 * 
	 * 此处重试读写测试通过   要有优先级相同的三台机器才能保证  三次重试  否则130行 抛出异常 
	 */
	protected <T> T tryExecuteInternal(Map<DataSource, SQLException> failedDataSources, DataSourceTryer<T> tryer,
		...
		//logger.info("===============================读写重试三次测试开始点=====================次数:"+times);
			...
			*/
			...
			try {
				if (dsHolder.isNotAvailable) {
					...
					//logger.info("===============================读写重试三次测试结束=====================次数:"+i);
					logger.info("数据源:"+dsHolder.dsw.getConnection().getMetaData().getURL());
